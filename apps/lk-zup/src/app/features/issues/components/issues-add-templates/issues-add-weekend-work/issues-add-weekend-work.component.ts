...
@Component({
    selector: 'app-issues-add-weekend-work',
    templateUrl: './issues-add-weekend-work.component.html',
    styleUrls: ['./issues-add-weekend-work.component.scss'],
    standalone: false
})
export class IssuesAddWeekendWorkComponent
  ...
  implements OnChanges, OnInit
{
  ...
  changePayType(event: any, payControlNumber: number, i: number): void {
    ...
    if (event.value === 'Дополнительный выходной') {
      ...
      this.weekendWork.controls.forEach((control) => {
        control.patchValue({
          dateBegin: control.value.dateBegin,
          payType1: 'Дополнительный выходной',
        });
      });
    }
  }
